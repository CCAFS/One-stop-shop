...
function loadItems(list) {
    ...
    $("#facetvalue").empty();
    tlist = document.getElementById('facetvalue');
    $.getJSON( "/get/"+list.value, function( data ) {
        data.forEach(function(entry) {
            ...
            /*$(entry.itemID).vectorMap({
                ...
                {
                    var cntyCode = code.toUpperCase();
                    if ((cntyCode == "ET") || (cntyCode == "IN") || (cntyCode == "PE") || (cntyCode == "IT"))
                    {
                        window.location = "/country/" + cntyCode;
                    }
                }
            });*/


            //new Chart(document.getElementById(entry.itemID).getContext("2d"), {type: 'doughnut', data: entry.data, options:doughnutOptions});
